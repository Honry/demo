...
function verifyHasPixels(data, init, wrap_around) {
  for (let i = init; i < data.length; i += 4) {
    if (data[i] > 0) {
      assert_less_than_equal(data[i], wrap_around);
      return;
    }
  }
  //data is all with zero values
  assert_false(true);
}

function verifyPixels(data, wrap_around, test_name) {
  if (test_name == "R32Float") {
    verifyHasPixels(data, 0, wrap_around);
  } else {
    for (let i = 0; i < 4; i++) {
      verifyHasPixels(data, i, wrap_around);
    }
  }
}
...
function runImageBitmapTest(bitmap, canvas, flip_y) {
  ...
  return verifyPixels(imageData.data, 255, "ImageBitmap");
}
...
function testVideoToTexture(gl, video, internalformat, format, type,
  ...
  for (let i in cases) {
    ...
    uploadVideoToTexture2D(gl, video, internalformat, format, type, flip_y,
                           premultiply);
    readAndVerifyFunction(gl, video.width, video.height);
...
function readAndVerifyRGBA8(gl, width, height) {
  let arr = new Uint8Array(width * height * 4);
  gl.readPixels(0, 0, width, height, gl.RGBA, gl.UNSIGNED_BYTE, arr);
  return verifyPixels(arr, 255, "RGBAUint8");
}

function readAndVerifyRGBA32F(gl, width, height) {
  let arr = new Float32Array(width * height * 4);
  gl.readPixels(0, 0, width, height, gl.RGBA, gl.FLOAT, arr);
  return verifyPixels(arr, 1.0, "RGBAFloat");
}

function readAndVerifyR32F(gl, width, height) {
  let arr = new Float32Array(width * height * 4);
  gl.readPixels(0, 0, width, height, gl.RGBA, gl.FLOAT, arr);
  return verifyPixels(arr, 1.0, "R32Float");
}
    ...
    .then(video => {
      ...
      let p2 = createImageBitmap(video,
        {imageOrientation: "none", premultiplyAlpha: "premultiply"}).then(
            function(imageBitmap) {
              runImageBitmapTest(imageBitmap, canvas, false); });
      ...
      let p5 = createImageBitmap(video,
        {imageOrientation: "flipY", premultiplyAlpha: "premultiply"}).then(
            function(imageBitmap) {
              runImageBitmapTest(imageBitmap, canvas, true); });
...
function getBothStreamsAndCheckForFeaturesPresence(deviceLabel, videoDeviceLabel) {
  ...
  }, "test feature presence when getting both color and depth streams");
}
